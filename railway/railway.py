...
class Trip:
    ...
    def __call__(self):
        if self.passengers:
            all_wigth = sum([passenger.load_weight for passenger in self.passengers])
            remain_weight = self.train.weight_capacity - all_wigth
...
class Passenger:
    ...
    def attend_trip(self, trip):
        remain_weight = trip()
        if remain_weight >= self.load_weight:
            trip.passengers.append(Passenger)
        else:
            return 'Heavy load!'

    def cancel_trip(self, trip):
        passenger = [passenger for passenger in trip.passengers if passenger.fullname == self.fullname]
        if passenger:
            del trip.passengers[passenger]
        else:
